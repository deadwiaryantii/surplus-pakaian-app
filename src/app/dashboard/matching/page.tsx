...
import { GitCompare, CheckCircle2, ArrowRight, Package, Building2 } from "lucide-react";
...
export default function MatchingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Stock & Requirement Matching</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400">Pencocokan otomatis stok gudang yang tersedia dengan profil kebutuhan penerima.</p>
      </div>
              ...
              {matchList.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                  ...
                  <td className="py-4 px-6 text-slate-600 dark:text-slate-400 flex items-center gap-1.5 mt-2">
                    <Package className="w-3.5 h-3.5 text-emerald-600" /> {item.stokTersedia}
                  </td>
