...
import { Package, MapPin, CheckCircle, Plus } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Monitoring Stok Layak (Stok Good)</h2>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Pencatatan pakaian lolos QC beserta jumlah, lokasi gudang, dan sumber donatur.</p>
        </div>
        <button
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                  <td className="py-4 px-6 font-medium text-slate-800 dark:text-slate-200">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="truncate">{item.barang}</span>
                    </div>
                  </td>
