...
import { Truck, Plus, Hash } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Delivery Order (DO) & Pengeluaran Stok</h2>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Pembuatan dokumen DO dengan UUID, resi ekspedisi, dan pengurangan stok.</p>
        </div>
        <button
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                  <td className="py-4 px-6 font-mono font-semibold text-emerald-600 dark:text-emerald-400">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate">{item.doNumber}</span>
                    </div>
                  </td>
