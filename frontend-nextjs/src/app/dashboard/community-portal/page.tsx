...
import { FileText, Plus, Hash, CheckCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Community Request Portal</h2>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Portal permintaan pakaian oleh komunitas penerima beserta unggah proposal PDF.</p>
        </div>
        <button
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                  ...
                  <td className="py-4 px-6">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                      ...
                      <span className="truncate max-w-[150px]">{item.proposal}</span>
                    </span>
                  </td>
                  <td className="py-4 px-6">
